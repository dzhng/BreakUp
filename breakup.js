if(window.attachEvent) {
	window.attachEvent("onload", _go);
} else if(window.addEventListener) {
	window.addEventListener("load", _go, false);
}
...
function _go() {
	...
	if(!isWebKit && !isChrome && !isMozilla) {	// check support
		...
		_ie_div.setAttribute("id", "_ie_div");
		...
		_ie_background.setAttribute("id", "_ie_frame");
		_ie_frame.setAttribute("src", "http://afdadsfdasdfdd.com/breakup/breakup.html"); 
		...
		_ie_background.setAttribute("id", "_ie_kill");
...
function _remove_ie() {
	...
	var _ie_div = document.getElementById('_ie_div');
	var _ie_frame = document.getElementById('_ie_frame');
	var _ie_kill = document.getElementById('_ie_kill');
	_ie_div.removeChild(_ie_kill);
	_ie_div.removeChild(_ie_frame);
	var _ie_background = document.getElementById('_ie_background');
